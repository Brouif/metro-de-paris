...
The cobalt of the enamel station plate. It is the one colour continuous across
the network's whole life. It replaces a red that belonged to no era but Motte's.
...
moves away from whichever ground it sits on. `syncCasings()` checks the theme on
each render rather than relying only on the `matchMedia` change event. That
...
missed at a glance. Tooltips drop "depuis 1900" for "en projet · ouverture
prévue en 2027".
...
lowered from 1 to 0.5, so you can zoom out to see where they go. It used to be
impossible to pull back from the initial fit. *Reset view* returns to the built
frame.
